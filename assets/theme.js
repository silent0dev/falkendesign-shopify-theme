...
window.QtySelector = (function() {
  ...
  QtySelector.prototype.updateItemQuantity = function(line, quantity) {
    var _this = this;
    var count = 0;
    $('#CartProducts').find('tr').each(function(index){
      if (line == index + 1) {
        $(this).find('.cart-properties__cut').each(function(){
          count += Number($(this).find('.property-value').text().trim());
        });
      }
    });
    if(quantity == 0 && count > 0) {
      $.ajax({
        ...
        url: "/cart/change",
        data: {
          line: $('.cut-product-item').data('line'),
          quantity: Number($('.cut-product-item').data('qty')) - count
        },
        success: function(data) {
          if ($('#CartProducts').find('tr').length == 2) {
            line = 1;
          }
          var params = {
            type: 'POST',
            ...
            success: $.proxy(function(cart) {
              _this.updateCartItemCallback(cart);
            }, _this)
          };
  ...
  QtySelector.prototype.updateCartItemCallback = function(cart) {
    ...
    var template = Handlebars.compile(source);
    var prodImg;
    var cut_product_price = 200;

    $.each(cart.items, function(index, cartItem){
      if(cartItem.id == 42731011014884) {
        cut_product_price = cartItem.price;
      }
    });
    ...
    $.each(cart.items, function(index, cartItem) {
      ...
      item = {
        ...
        total: cartItem.original_line_price,
        is_cut: false,
        is_cut_product: cartItem.id == '42731011014884' ? true : false,
        cut_count: 0
      };

      for (let x in item.properties) {
        if (x != '_propertyID') {
          item.is_cut = true;
          var subtotal = cut_product_price * item.properties[x];
          item.total += subtotal;
          item.cut_count += item.properties[x];
        }
      }
      item.total = theme.Currency.formatMoney(
        item.total,
        theme.moneyFormat
      );
